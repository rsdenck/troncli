...
"github.com/rsdenck/nux/internal/console"
	...
	"github.com/rsdenck/nux/internal/modules/network"
...
var netInfoCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		table.SetTitle("NUX - INFORMAÇÕES DE REDE")
...
var netSocketCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle("NUX - SOCKETS ABERTOS")
...
var netTraceCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle(fmt.Sprintf("NUX - TRACEROUTE: %s", args[0]))
...
var netDigCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		lines := strings.Split(res, "\n")
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle(fmt.Sprintf("NUX - DNS LOOKUP: %s", args[0]))
...
var netNmapCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle(fmt.Sprintf("NUX - PORT SCAN: %s", args[0]))
...
var netTcpdumpCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		lines := strings.Split(res, "\n")
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle(fmt.Sprintf("NUX - PACKET CAPTURE: %s", args[0]))
