...
"github.com/rsdenck/nux/internal/console"
	...
	"github.com/rsdenck/nux/internal/modules/disk"
...
var diskUsageCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) == 0 {
			...
			table.SetTitle("NUX - USO DE DISCO (DEVICES)")
			...
			table := console.NewBoxTable(os.Stdout)
			table.SetTitle(fmt.Sprintf("NUX - USO DE DISCO: %s", args[0]))
...
var diskHealthCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle("NUX - SAÚDE DO DISCO")
...
var diskTopFilesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle(fmt.Sprintf("NUX - TOP %d ARQUIVOS: %s", count, path))
...
var diskInodesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		table := console.NewBoxTable(os.Stdout)
		table.SetTitle(fmt.Sprintf("NUX - INODES: %s", path))
		table.SetHeaders([]string{"METRIC", "VALUE"})
